// RUN: not %clang_cc1 -emit-llvm -main-file-name %s 2>&1 | FileCheck %s --check-prefix=ERR1 --implicit-check-not=error:
...
// RUN: %clang_cc1 -main-file-name %s -emit-llvm -o /dev/null
