...
// RUN: %clang -fintegrated-cc1 -fintegrated-as -target x86_64 -c -### %s 2>&1 | FileCheck %s --check-prefix=YES
...
// RUN: %clang -fno-integrated-cc1 -fintegrated-cc1 -fintegrated-as -target x86_64 -c -### %s 2>&1 \
// RUN:     | FileCheck %s --check-prefix=YES

// RUN: %clang_cl -fintegrated-cc1 -fintegrated-as -target x86_64 -c -### -- %s 2>&1 \
// RUN:     | FileCheck %s --check-prefix=YES
// RUN: %clang_cl -fno-integrated-cc1 -c -### -- %s 2>&1 \
// RUN:     | FileCheck %s --check-prefix=NO

// RUN: env CCC_OVERRIDE_OPTIONS=+-fintegrated-cc1 \
// RUN:     %clang -fintegrated-cc1 -fintegrated-as -target x86_64 -c -### %s 2>&1 \
// RUN:     | FileCheck %s --check-prefix=YES
...
// RUN: echo > %t.s
// RUN: %clang --target=x86_64-linux -fintegrated-cc1 -fintegrated-as -target x86_64 -c -### %t.s 2>&1 | FileCheck %s --check-prefix=YES
