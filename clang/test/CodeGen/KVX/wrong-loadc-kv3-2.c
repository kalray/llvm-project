...
void wrong_ready(int sv, void * addr, unsigned long cond, v4i64 big_sv, v2i64 mid_sv){
  ...
  __builtin_kvx_loadc256(big_sv, addr, cond, ".toto"); // expected-error {{Expects either a lsucond: ['.wlez', '.wnez', '.weqz', '.dgez', '.wgtz', '.even', '.dlez', '.dnez', '.wltz', '.deqz', '.odd', '.dgtz', '.wgez', '.dltz'] or a lsomask: ['.mt', '.mf', '.mtc', '.mfc']}} expected-error {{cannot compile this builtin function yet}}
}
