...
void wrong_load(s128 *s, v8i64 v, int sv, void * addr, unsigned long cond){
  ...
  __builtin_kvx_loadcwz(sv, addr, cond, ".toto"); // expected-error {{Expects a lsucond, one of: '.wlez', '.wnez', '.weqz', '.dgez', '.wgtz', '.even', '.dlez', '.dnez', '.wltz', '.deqz', '.odd', '.dgtz', '.wgez', '.dltz'}} expected-error {{cannot compile this builtin function yet}}
