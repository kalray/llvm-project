...
static cl::opt<bool> DisableLoadStorePacking(
    "disable-kvx-loadstore-packing", cl::Hidden,
    cl::desc("Disable Load/Store Packing Pass for KVX target"));

extern "C" void LLVMInitializeKVXTarget() {
...
KVXTargetMachine::KVXTargetMachine(const Target &T, const Triple &TT,
    ...
    : LLVMTargetMachine(T,
      ...
      TLOF(make_unique<KVXELFTargetObjectFile>()) {
  initAsmInfo();
}
...
KVXTargetMachine::getSubtargetImpl(const Function &F) const {
  ...
  if (!I) {
    ...
    I = llvm::make_unique<KVXSubtarget>(TargetTriple, CPU, FS, *this);
  }
  return I.get();
}
