  ...
  for (auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      /* const unsigned VReg =
       ...
       int FI = MF.getFrameInfo().CreateFixedObject(8, 0, true);
       auto PtrVT = getPointerTy(MF.getDataLayout());
       SDValue ArgIn = DAG.getStore(Chain, DL, VArg, DAG.getFrameIndex(FI,
       PtrVT), MachinePointerInfo::getFixedStack(MF,FI));*/
