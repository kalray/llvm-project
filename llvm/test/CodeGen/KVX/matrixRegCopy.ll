; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc -o - %s | FileCheck %s
target triple = "kvx-kalray-cos"

define void @c([512 x float]* %0, [512 x float]* %1, [768 x half]* %2, [512 x half]* %3) {
; CHECK-LABEL: c:
; CHECK:       # %bb.0:
; CHECK-NEXT:    mt44d $a0a1a2a3 = $a0a1a2a3
; CHECK-NEXT:    compd.ltu $r0 = $r0, 508
; CHECK-NEXT:    ;;
; CHECK-NEXT:  .LBB0_1: # =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    copyv $a4 = $a0
; CHECK-NEXT:    ;;
; CHECK-NEXT:    copyv $a5 = $a1
; CHECK-NEXT:    ;;
; CHECK-NEXT:    copyv $a6 = $a2
; CHECK-NEXT:    ;;
; CHECK-NEXT:    copyv $a7 = $a3
; CHECK-NEXT:    ;;
; CHECK-NEXT:    lv.c3 $a4a5a6a7 = 0[$r0]
; CHECK-NEXT:    ;;
; CHECK-NEXT:    lv.c3 $a4a5a6a7 = 0[$r0]
; CHECK-NEXT:    cb.odd $r0 ? .LBB0_1
; CHECK-NEXT:    ;;
; CHECK-NEXT:  # %bb.2:
; CHECK-NEXT:    ret
; CHECK-NEXT:    ;;
  br label %5

5:
  br label %6

6:
  %7 = phi <1024 x i1> [ undef, %5 ], [ %19, %18 ]
  %8 = tail call <1024 x i1> @llvm.kvx.lvc(<1024 x i1> %7, i8* nonnull undef, i32 3, i32 0)
  %9 = or i64 undef, 2
  br label %10

10:
  %11 = bitcast half* undef to i8*
  %12 = getelementptr inbounds [768 x half], [768 x half]* %2, i64 undef, i64 undef
  %13 = tail call <512 x i1> @llvm.kvx.fmma242hw(<512 x i1> undef, <256 x i1> undef, <256 x i1> undef)
  %14 = tail call <1024 x i1> @llvm.kvx.lvc(<1024 x i1> undef, i8* nonnull undef, i32 3, i32 0)
  %15 = tail call <256 x i1> @llvm.kvx.movefmv(<1024 x i1> undef, i32 1)
  %16 = tail call <512 x i1> @llvm.kvx.fmma242hw(<512 x i1> undef, <256 x i1> undef, <256 x i1> undef)
  %17 = tail call <512 x i1> @llvm.kvx.fmma242hw(<512 x i1> undef, <256 x i1> undef, <256 x i1> undef)
  br i1 undef, label %10, label %18

18:
  %19 = tail call <1024 x i1> @llvm.kvx.mt44d(<1024 x i1> undef)
  %20 = tail call <256 x i1> @llvm.kvx.movefmv(<1024 x i1> %19, i32 0)
  %21 = icmp ult i64 undef, 508
  br i1 %21, label %6, label %22

22:
  br i1 undef, label %5, label %23

23:
  ret void
}

declare <1024 x i1> @llvm.kvx.lvc(<1024 x i1>, i8*, i32, i32)
declare <512 x i1> @llvm.kvx.movefmw(<1024 x i1>, i32)
declare <256 x i1> @llvm.kvx.movefmv(<1024 x i1>, i32)
declare <512 x i1> @llvm.kvx.fmma242hw(<512 x i1>, <256 x i1>, <256 x i1>)
declare <1024 x i1> @llvm.kvx.buildfwm(<512 x i1>, <512 x i1>)
declare <1024 x i1> @llvm.kvx.mt44d(<1024 x i1>)
