; NOTE: Assertions have been autogenerated by utils/update_llc_test_checks.py
; RUN: llc --frame-pointer=none < %s | FileCheck -check-prefix=FP-NONE %s
; RUN: llc --frame-pointer=all < %s | FileCheck -check-prefix=FP-ALL %s

target triple = "kvx-kalray-cos"

; int g(int*, int*);
;
; int f(int sz) {
;   int *a = __builtin_alloca_with_align(sz, 1024);
;   int *b = __builtin_alloca_with_align(8, 512);
;   g(a, b);
;   return *a;
; }

define i32 @f(i32 %sz) {
; FP-NONE-LABEL: f:
; FP-NONE:       # %bb.0: # %entry
; FP-NONE-NEXT:    addd $r12 = $r12, -384
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_def_cfa_offset 384
; FP-NONE-NEXT:    copyd $r32 = $r12
; FP-NONE-NEXT:    addd $r12 = $r12, 96
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_def_cfa_register 32
; FP-NONE-NEXT:    andd $r12 = $r12, -128
; FP-NONE-NEXT:    copyd $r33 = $r14
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    copyd $r14 = $r12
; FP-NONE-NEXT:    get $r16 = $ra
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    sd 24[$r12] = $r16
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_escape 0x10, 0x43, 0x02, 0x7e, 0x18
; FP-NONE-NEXT:    sd 16[$r12] = $r33
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_escape 0x10, 0x0e, 0x02, 0x7e, 0x10
; FP-NONE-NEXT:    sd 8[$r12] = $r31
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_escape 0x10, 0x1f, 0x02, 0x7e, 0x08
; FP-NONE-NEXT:    sd 0[$r12] = $r18
; FP-NONE-NEXT:    copyd $r31 = $r32
; FP-NONE-NEXT:    sxwd $r0 = $r0
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_escape 0x10, 0x12, 0x02, 0x7e, 0x00
; FP-NONE-NEXT:    .cfi_def_cfa_register 31
; FP-NONE-NEXT:    addd $r0 = $r0, 31
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    andd $r0 = $r0, -32
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    sbfd $r0 = $r0, $r12
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    andd $r18 = $r0, -128
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    copyd $r12 = $r18
; FP-NONE-NEXT:    addd $r1 = $r14, 128
; FP-NONE-NEXT:    copyd $r0 = $r18
; FP-NONE-NEXT:    call g
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    lwz $r0 = 0[$r18]
; FP-NONE-NEXT:    copyd $r32 = $r31
; FP-NONE-NEXT:    copyd $r12 = $r14
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_def_cfa_register 32
; FP-NONE-NEXT:    ld $r18 = 0[$r12]
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    ld $r31 = 8[$r12]
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    ld $r16 = 24[$r12]
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    set $ra = $r16
; FP-NONE-NEXT:    ld $r14 = 16[$r12]
; FP-NONE-NEXT:    copyd $r12 = $r32
; FP-NONE-NEXT:    ;;
; FP-NONE-NEXT:    .cfi_restore 67
; FP-NONE-NEXT:    .cfi_def_cfa_register 12
; FP-NONE-NEXT:    addd $r12 = $r12, 384
; FP-NONE-NEXT:    ret
; FP-NONE-NEXT:    ;;
;
; FP-ALL-LABEL: f:
; FP-ALL:       # %bb.0: # %entry
; FP-ALL-NEXT:    addd $r12 = $r12, -384
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_def_cfa_offset 384
; FP-ALL-NEXT:    copyd $r32 = $r12
; FP-ALL-NEXT:    addd $r12 = $r12, 96
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_def_cfa_register 32
; FP-ALL-NEXT:    andd $r12 = $r12, -128
; FP-ALL-NEXT:    copyd $r33 = $r14
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    copyd $r14 = $r12
; FP-ALL-NEXT:    get $r16 = $ra
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    sd 24[$r12] = $r16
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_escape 0x10, 0x43, 0x02, 0x7e, 0x18
; FP-ALL-NEXT:    sd 16[$r12] = $r33
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_escape 0x10, 0x0e, 0x02, 0x7e, 0x10
; FP-ALL-NEXT:    sd 8[$r12] = $r31
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_escape 0x10, 0x1f, 0x02, 0x7e, 0x08
; FP-ALL-NEXT:    sd 0[$r12] = $r18
; FP-ALL-NEXT:    copyd $r31 = $r32
; FP-ALL-NEXT:    sxwd $r0 = $r0
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_escape 0x10, 0x12, 0x02, 0x7e, 0x00
; FP-ALL-NEXT:    .cfi_def_cfa_register 31
; FP-ALL-NEXT:    addd $r0 = $r0, 31
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    andd $r0 = $r0, -32
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    sbfd $r0 = $r0, $r12
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    andd $r18 = $r0, -128
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    copyd $r12 = $r18
; FP-ALL-NEXT:    addd $r1 = $r14, 128
; FP-ALL-NEXT:    copyd $r0 = $r18
; FP-ALL-NEXT:    call g
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    lwz $r0 = 0[$r18]
; FP-ALL-NEXT:    copyd $r32 = $r31
; FP-ALL-NEXT:    copyd $r12 = $r14
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_def_cfa_register 32
; FP-ALL-NEXT:    ld $r18 = 0[$r12]
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    ld $r31 = 8[$r12]
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    ld $r16 = 24[$r12]
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    set $ra = $r16
; FP-ALL-NEXT:    ld $r14 = 16[$r12]
; FP-ALL-NEXT:    copyd $r12 = $r32
; FP-ALL-NEXT:    ;;
; FP-ALL-NEXT:    .cfi_restore 67
; FP-ALL-NEXT:    .cfi_def_cfa_register 12
; FP-ALL-NEXT:    addd $r12 = $r12, 384
; FP-ALL-NEXT:    ret
; FP-ALL-NEXT:    ;;
entry:
  %conv = sext i32 %sz to i64
  %0 = alloca i8, i64 %conv, align 128
  %1 = bitcast i8* %0 to i32*
  %2 = alloca [2 x i32], align 64
  %.sub = getelementptr inbounds [2 x i32], [2 x i32]* %2, i64 0, i64 0
  %call = call i32 @g(i32* nonnull %1, i32* nonnull %.sub)
  %3 = load i32, i32* %1, align 128
  ret i32 %3
}

declare i32 @g(i32*, i32*)
